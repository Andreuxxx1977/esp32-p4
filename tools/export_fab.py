...
def fab_notes(board: Path, drc: Path | None) -> str:
    ...
    L = ["# ESP32-P4 Extreme -- fabrication package", "",
         "> **Prototype files, generated by an autorouter. Not fabricated or bench-tested yet.**"]
    if blocking == []:
        L += ["> The copper is fully connected and passes KiCad's DRC for manufacturing and electrical",
              "> rules, but the high-speed routing rules listed at the end are **not** met (an",
              "> autorouter does not route coupled differential pairs). USB 2.0 High-Speed, MIPI",
              "> CSI/DSI and Ethernet may not work reliably on this revision. Review before ordering."]
    else:
    ...
    if report is not None:
        ...
        L += ["## Design-rule check (KiCad 10)", "",
              ...
              "- High-speed rules **not met** (see the warning at the top):", ""]
